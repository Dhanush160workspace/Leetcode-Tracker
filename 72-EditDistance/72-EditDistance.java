// Last updated: 7/9/2026, 3:11:00 PM
class Solution {
    public int minDistance(String word1, String word2) {
        int m = word1.length();
        int n = word2.length();
        int dp[][] = new int[m+1][n+1];
        for (int row[]:dp){
            Arrays.fill(row, -1);
        }
        int ans = helper(word1, word2, m, n, dp);
        return ans;
    }
    public int helper(String ch1, String ch2, int m, int n, int dp[][]){
        if (m==0) return n;
        if (n==0) return m;
        if (ch1.charAt(m-1) == ch2.charAt(n-1)){
            int ans = 0 + helper(ch1, ch2, m-1, n-1, dp);
            dp[m][n] = ans;
            return ans;
        }
        if (dp[m][n] != -1){
            return dp[m][n];
        }
        int delete = 1 + helper(ch1, ch2, m-1, n, dp);
        int replace = 1 + helper(ch1, ch2, m-1, n-1, dp);
        int insert = 1 + helper(ch1, ch2, m, n-1, dp);
        dp[m][n] = Math.min(Math.min(replace, insert), delete);
        return dp[m][n];
    }
}